...
let EditTo = false;
class ProductSettingUpdate extends React.Component {
	static propTypes = {
	...
	componentDidMount() {
		...
		let { id, name, description, price, supplier, category, stock, image } =
			this.props.product;
		this.setState({
			name,
			description,
	...
	render() {
		...
		const { onChange, isImageChanged, onUpdateSubmit, onEditCloseButton } =
			this;
		let { suppliers, categories } = this.props;
		return (
			<>
				<div class="h-screen ">
						...
						>
							...
							<div className="h-full overflow-auto w-full flex flex-col">
								<div className="m-2 md:m-12">
									<form onSubmit={onUpdateSubmit(id)}>
